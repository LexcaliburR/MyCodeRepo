...
class Visual:
    ...
    def _cluster(self):
        ...
        tmp_board = np.array(self.board_points)
        self.board_points = []
        for i in range(max_label + 1):
            cluster = tmp_board[labels == i]
            self.board_points.append(cluster)

        self.viz.add_geometry(pcd)

    def _compute_line1(self):
        boards = self.board_points
        ...
        for pts in boards:
            ...
            # print(f"k: {model.coef_[0]},  b: {model.intercept_}")
            #
            vis_pts = [[model.predict([[-15]])[0][0], -15, 0], [model.predict([[15]])[0][0], 15, 0]]
            vis_pts = [[-15, -15 * model.coef_[0] + model.intercept_, 0], [15, 15 * model.coef_[0] + model.intercept_, 0]]
            a1 = model.coef_[0]
            b1 = model.intercept_
            ...
            vis_pts = [[x1, y1, 0], [x2, y2, 0]]
            viz_line = [[0, 1]]

            colors = [[0, 0, 1] for i in range(len(viz_line))]

            line_set = o3d.geometry.LineSet(
                points=o3d.utility.Vector3dVector(vis_pts),
                lines=o3d.utility.Vector2iVector(viz_line)
            )
            line_set.colors = o3d.utility.Vector3dVector(colors)
            # line_set.line_width = 10
            self.viz.add_geometry(line_set)
